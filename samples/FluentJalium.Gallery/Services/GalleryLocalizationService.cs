...
internal sealed class GalleryLocalizationService
{
    ...
    private static Dictionary<string, Dictionary<string, string>> CreateLocalizedText()
    {
        ...
        AddPage(text, "textinput", "Text Input", "文本输入", "TextBox, PasswordBox, NumberBox, AutoCompleteBox, AutoSuggestBox, and RichTextBox surfaces with states, filtering, query submission, suggestion choice, and material input panels.", "TextBox、PasswordBox、NumberBox、AutoCompleteBox、AutoSuggestBox 与 RichTextBox 的状态、筛选、查询提交、建议选择和材质输入面板。", "text input textbox password number autocomplete autosuggest query submitted suggestion chosen rich 文本 输入 密码 数字 自动完成 自动建议");
        ...
        AddPage(text, "forms", "Forms", "表单", "App-pattern form scenarios that combine labels, text input, AutoSuggestBox, RadioButtons, InfoBar validation, SettingsCard rows, switches, and submit/reset commands.", "组合标签、文本输入、AutoSuggestBox、RadioButtons、InfoBar 验证、SettingsCard 行、开关以及提交/重置命令的应用表单场景。", "forms validation label text input autosuggest radiobuttons infobar settingscard toggleswitch submit reset required 表单 验证 标签 输入 自动建议 单选 信息栏 设置 提交 重置 必填");
        ...
        AddPage(text, "contentandlayout", "Content and Layout", "内容与布局", "TextBlock, AccessText, Border, content hosts, panels, Grid, SettingsCard command rows, transitioning content, and LiquidGlass layout surfaces.", "TextBlock、AccessText、Border、内容宿主、面板、Grid、SettingsCard 命令行、过渡内容和 LiquidGlass 布局表面。", "content layout grid panel wrap settings command canexecute 内容 布局 网格 面板 设置 命令");
        ...
        AddPage(text, "collections", "Collections", "集合", "ListBox, ListView, TreeView, DataGrid, and TreeDataGrid controls with empty, loading, grouped, density, hierarchy, and material data states.", "ListBox、ListView、TreeView、DataGrid 和 TreeDataGrid 的空态、加载态、分组、密度、层级和材质数据状态。", "collections list tree data grid empty loading grouped density compact 集合 列表 树 数据 表格 空态 加载 分组 密度 紧凑");
        ...
        AddPage(text, "windowbackdrops", "Window Backdrops", "窗口背景", "DWM-backed shell materials and FWFluentWindowSurface profiles for Jalium windows, including Mica, Mica Alt, Acrylic, and solid shell fallback.", "Jalium 窗口的 DWM 外壳材质与 FWFluentWindowSurface 配置，包括 Mica、Mica Alt、Acrylic 和纯色外壳回退。", "window backdrop FWFluentWindowSurface mica acrylic 窗口 背景 云母 亚克力");
        AddPage(text, "materialsand effects".Replace(" ", string.Empty), "Materials and Effects", "材质与效果", "Element backdrops, typed FW material surfaces, material roles, and WinUI-style layering for FluentJalium surfaces.", "FluentJalium 表面的元素背景、FW 派生材质控件、材质角色和 WinUI 风格分层。", "materials effects FWLayerSurface FWMicaSurface FWAcrylicSurface FWCardSurface FWFlyoutSurface glass 材质 效果 亚克力 云母 玻璃");
        ...
        AddPage(text, "disclosure", "Disclosure", "披露", "Expander, SettingsExpander, ToolTip, TeachingTip, ContentDialog, TaskDialog, TaskDialogHost, and GroupBox controls with item-host rows, target placement, modal overlays, command states, escape/cancel behavior, focus restore/trap, and LiquidGlass disclosure panels.", "Expander、SettingsExpander、ToolTip、TeachingTip、ContentDialog、TaskDialog、TaskDialogHost 和 GroupBox 的设置项宿主、目标定位、模态遮罩、命令状态、Escape/取消行为、焦点恢复/陷阱与 LiquidGlass 披露面板。", "disclosure expander settingsexpander itemcount tooltip teachingtip dialog taskdialog taskdialoghost diagnostics keyboard request tab shift tab modal overlay command escape focus restore trap light dismiss groupbox 披露 展开 设置 提示 教学提示 对话框 模态 遮罩 命令 焦点 分组");
        ...
        AddPage(text, "status", "Status", "状态", "InfoBar, InfoBadge, Snackbar, ToastNotification, and StatusBar controls with severity, service queues, close reasons, pauseable lifetimes, host placement, spacing, overlay/root hosts, transition hooks, diagnostics, and material operation states.", "InfoBar、InfoBadge、Snackbar、ToastNotification 和 StatusBar 的严重性、服务队列、关闭原因、可暂停生命周期、宿主位置、间距、覆盖层/根宿主、过渡钩子、诊断状态和材质操作状态。", "status notification info toast snackbar service close reason pause hover focus placement spacing overlay root host popup transition diagnostics badge 状态 通知 信息 徽章 服务 关闭原因 暂停 位置 间距 覆盖层 根宿主 弹出 过渡 诊断");
        ...
        AddPage(text, "controlgapmatrix", "Control Gap Matrix", "控件缺口矩阵", "Diagnostic backlog matrix that tracks which Fluent Design controls are public FW surfaces, Gallery recipes, Evaluate candidates, or rendered-QA gates across WinUI, WPF UI, UI.WPF.Modern, FluentAvalonia, and Community Toolkit references.", "跟踪 Fluent Design 控件在 WinUI、WPF UI、UI.WPF.Modern、FluentAvalonia 和 Community Toolkit 参考下是公共 FW 表面、Gallery recipe、Evaluate 候选还是 rendered-QA 门禁的诊断待办矩阵。", "control gap matrix backlog WinUI WPFUI UI.WPF.Modern FluentAvalonia Community Toolkit public FW recipe evaluate rendered QA diagnostics 控件 缺口 矩阵 待办 诊断");
        AddPage(text, "visualqacoverage", "Visual QA Coverage", "视觉 QA 覆盖", "Diagnostic matrix for FluentJalium control-family state coverage, evidence, and registered sample-code keys.", "FluentJalium 控件家族状态覆盖、证据和已注册示例代码键的诊断矩阵。", "visual qa coverage diagnostics state evidence sample key family normal hover pressed selected disabled focus density high contrast 视觉 覆盖 诊断 状态 证据");
