...
internal sealed class GalleryLocalizationService
{
    ...
    private static Dictionary<string, Dictionary<string, string>> CreateLocalizedText()
    {
        ...
        AddPage(text, "textinput", "Text Input", "文本输入", "TextBox, PasswordBox, NumberBox, AutoCompleteBox, and RichTextBox surfaces with states, filtering, and material input panels.", "TextBox、PasswordBox、NumberBox、AutoCompleteBox 与 RichTextBox 的状态、筛选和材质输入面板。", "text input textbox password number autocomplete rich 文本 输入 密码 数字 自动完成");
