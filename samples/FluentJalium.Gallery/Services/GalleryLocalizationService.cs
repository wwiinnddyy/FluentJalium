...
internal sealed class GalleryLocalizationService
{
    ...
    private static Dictionary<string, Dictionary<string, string>> CreateLocalizedText()
    {
        ...
        AddPage(text, "disclosure", "Disclosure", "披露", "Expander, SettingsExpander, ToolTip, TeachingTip, ContentDialog, TaskDialog, and GroupBox controls with item-host rows, target placement, command states, escape/cancel behavior, and LiquidGlass disclosure panels.", "Expander、SettingsExpander、ToolTip、TeachingTip、ContentDialog、TaskDialog 和 GroupBox 的设置项宿主、目标定位、命令状态、Escape/取消行为与 LiquidGlass 披露面板。", "disclosure expander settingsexpander itemcount tooltip teachingtip dialog taskdialog command escape focus groupbox 披露 展开 设置 提示 教学提示 对话框 命令 分组");
