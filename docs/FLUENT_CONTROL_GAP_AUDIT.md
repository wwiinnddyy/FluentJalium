...
- `FWItemsRepeater`: explicit range realization, recycle-pool reuse, realized-index lookup, viewport-derived windows, axis-aware cache selection, and diagnostics now exist; remaining work is deeper ScrollViewer/FWScroller lifecycle integration on top of the current `Panel`-based API.
- Navigation shell: `FWNavigationService`, `FWNavigationRoute`, `FWNavigationServiceDiagnostics`, and `FWNavigationViewItem.RouteKey` now cover WPF UI-style route registration, NavigationView selection coordination, Frame back/forward history, and service diagnostics; remaining work is optional page-provider/type-factory injection for dependency-injected app shells.
