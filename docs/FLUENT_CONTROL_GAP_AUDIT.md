...
- Advanced interaction diagnostics coverage: `FWScrollerViewportDiagnostics` is now visible in the Advanced Interaction Gallery sample and registry code, showing attached viewport, offset, viewport, and extent metrics alongside snap/scroll controls.
- RefreshContainer diagnostics coverage: `FWRefreshContainer` now exposes `FWRefreshContainerDiagnostics` for pull direction, refresh/pull state, pull distance, threshold, progress, template attachment, and visualizer state, with the Advanced Interaction Gallery and registry code surfacing the snapshot.
- AnnotatedScrollBar diagnostics coverage: `FWAnnotatedScrollBar` now exposes `FWAnnotatedScrollBarDiagnostics`, tracks registered labels independently of template realization, records the last requested detail label, and surfaces label/range/value diagnostics in Gallery and registry samples.
