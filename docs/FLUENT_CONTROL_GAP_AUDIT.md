...
- Navigation shell: `FWNavigationService`, `FWNavigationRoute`, `FWNavigationServiceDiagnostics`, `FWNavigationViewItem.RouteKey`, and `FWNavigationPageTypeProvider` now cover WPF UI-style route registration, NavigationView selection coordination, Frame back/forward history, service diagnostics, and dependency-injected page resolution; remaining work is visual QA for larger shell compositions.
