...
- 不声称"原生控件不吃模板 / 只能自绘"这条普查结论现在普遍可推：S0-p 的名单给出 93 个自带 `OnRender` 的导出类型，
  其中本库重模板或重上色的 20 个里**只有 `InfoBar` 一例**按"与 `OnRender` 覆写成空的同类逐像素相同"量过让路。
...
- 不声称间距已逐控件与上游量化：本轮目视逐页看过，但把 padding/margin/MinHeight 对到上游数值的只有
  InfoBar（`InfoBarContentRootPadding` 等 7 条 Thickness 逐条读取）与 Menu 条目（38 的 `MinHeight` 字面量），
  其余仍只到"转录时用的是上游度量行原值"这一层，没有逐控件的截图差分。
