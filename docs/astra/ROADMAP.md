...
**视觉缺陷批第三段（间距批，2026-09-19）**：用户抱怨的第二半——"间距不合 Fluent"——按上游逐条量。命令栏条目改成上游的三层形状（`Root` 无填充 / `AppBarButtonInnerBorder` 高亮 2,6,2,6 / `ContentRoot` 带 64），图标槽 `Height=16` + `0,16,0,2`（新发布行 `AppBarButtonContentViewboxCollapsedMargin`），Compact 同时写 2,6,2,22 与 48，`AppBarSeparator` 撤掉自造的 `MinHeight=40` 并把 2,8,2,8 还给矩形 margin；`ComboBoxItem` 模板根补上游那条 `5,2,5,2` 行内缩（`AstraComboBoxTests` 从排布尺寸读回：高亮宽 = 条目宽 − 10、高 = 条目高 − 4）；`Slider` 拇指改回上游 18×18 + 环 `Margin=-2` + 内芯 12，刻度条与轨道之间 4 的间隙连同 14/Auto/14 三段带一起照抄，并量出**框架给拇指留的是字面量 16**（220 控件行程 204，换 16 拇指与换 18 拇指两次同值），所以 18 的环在最大值处超出控件 2 DIP —— 写进 gap，而不是把拇指缩回 16 把它藏掉。这一批最值钱的一条是新的基座边界（`adaptation/00` **S0-r**）：**嵌套层的笔刷过渡终值进不了合成帧**——三层形状改完后，Gallery 里默认勾选的 `AppBarToggleButton` 只剩黑字没有 accent 底，而属性读回、离屏 `Render`、栏内离屏、上屏 `Chrome` 四条通路全说"填充在"，PrintWindow 读到 **0 px** `#60CDFF`；其余代码不动、只删那条 `TransitionProperty` 重建，同一坐标变 **7786 px**，而旧形状里同一个过渡在高亮层作为模板根时是落帧的（旧提交里的捕获即证）。两个 bar 按钮因此不带那 83ms 淡入——错色让位给观感——并给任务 #22 开出其余 10 处同类写法的逐条复核单（`Common.jalxaml` 三处、`Menus.jalxaml:291`、`Surfaces.jalxaml:45/62`、`Navigation.jalxaml:21/69`、`Inputs.jalxaml:27/34`）。四类证据：构建 Debug/Release 两通道 0 错误；行为/资源 = 全套 **620/620 全绿 0 skip**，Debug/Release 两通道各一遍（新增命令栏几何 1 条、栏内像素 2 条、上屏后置勾选 1 条、ComboBox 排布 2 条，并把 Slider 的行程与尺寸断言按实测改写，12 处部件名从 `Root` 迁到 `AppBarButtonInnerBorder`）；视觉 = `spike/VisualQA/out/command-bar.png`（PrintWindow，dpi=168）里 Bold 格子 7786 px 强调蓝、图标在标签之上、条目带 64 高；硬件输入 = 仍为零。调色板三档 checked=True（本批零改动）。 | 每批全 9 步 + 全闸口 |
