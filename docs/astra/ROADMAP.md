...
## #47 第二成员：帧闸——会飘的不是环，是"要 24 帧"这件事本身（目标项 5，2026-09-23）

`The_spin_moves_the_arc_and_stops_with_the_ring` 是家族里"两帧读到同值弧"那条。它的前提是 `PixelHarness.Pump`
同时受两个上限约束、先到先停：请求的帧数，和 400 ms 的墙上时钟预算。它一直把"到了几帧"算出来（`return seen`），
但 `Settle` 是 `void`，那个数被丢掉——所以一次在跑完全量的机器上交付 0 帧的 settle，读回来就是"环没动"。

改造分两步，第二步被第一步的量给推翻了：

1. `Settle` 交回交付帧数，另加 `SettleFrames(minimum)`：在捕获那条同样的三轮预算（1200 ms）里累计，直到帧真的到了。
   四条比较腿先都抬到"必须到齐 24 帧"。第一次定向跑就把**静止腿**弄红，红字逐字是
   `the stopped-ring leg compares two reads 24 rendered frames apart, and the pump delivered 19 within the settle budget`。
2. 那条红是判据自己的错：环一停，场景就是静止的，框架本来就不再发 `CompositionTarget.Rendering`（`Pump` 的注释里
   写着这件事），于是"向一个停下的环要 24 帧"是一条永远不成立的先决条件。反向做牙齿检查——把地板抬到不可达的 10000——
   运动腿报 `delivered 144 frame(s)`：**环在转的时候，同一个预算能要出 144 帧**。两个数一比，形状就清楚了：
   "经过了多少帧"在这个 harness 里根本不是常量，它由场景有没有活循环决定。

于是两条腿分开写，谁也不许借谁的证据：运动腿要的是"经过的渲染时间"，地板 2 帧（实测远多于），到不齐就红在**仪器**名下；
静止腿不再要帧（那是要一个设计上不来的状态），只把交付数写进失败消息，让下一次红能自证是"环被谁动了"还是"这一拍没帧"。
1561 那次红过之后没能再抓到现场，本批没有假装还原了它——它换来的是"下次红字会先问有没有渲染时间"。

顺带量到的两件事另立 #78：全库除 `FluentThemeManager` 自己以外没有任何源文件读 `ReduceMotion`，而
`Motion/ProgressRingAnimator.cs` 是硬编码 900 度 / 2 秒——我们的环不听减动效政策；页闸 status 档是 26 行里唯一
`stable=True/False` 的那两行、槽内色数逐跑飘（401 / 399 / 397 / 288），成因就是页里两枚 indeterminate 环一直在要帧，
飘的是环的相位，不是判据坏了（offender 判据不吃这个数，所以 PASS 仍然成立）。

**四类证据**：构建——本批只动测底座与测点，闸口读数见提交信息。行为：三次定向跑（环类 30/30 绿、环+菜单 140/140 绿、
突变跑如上文两条红字）。视觉：本批**没有**像素主张变动，也没重跑视觉测点。硬件输入：仍为零。
**不声称**：① 144 与 19 都是这台机器、这个窗口状态下的读数，不是框架常数，别拿它当阈值再抄一遍；② 全库 400+ 处
...
**本任务零代码改动**，所以它没有自己的闸口跑：普查是只读脚本，三条标红全部读体驳回，一条断言也没写。
...
| 串行闸口 | **最新一次（`spike/PagePixelsPlateDiag/gate-93.log`，提交 `feb45af` 的树）：整条管道全绿** —— 步骤依次是 restore、build（Debug，`0 个警告 / 0 个错误`）、套件 `失败: 0，通过: 1594，总计: 1594`（7 m 48 s）、页级像素 `PASS 13 pages x 3 variants, 0 offender(s)`（这一跑带着 #93 刚补的底板自证重编译过工具）、调色板漂移三档 `checked=True`、`keys.md is current: 1312 canonical lines.`、`known-gaps.md is current: 990 canonical lines.`，末尾打印 `All Astra gates passed.`，管道自己交出 `gate-exit=0`。上一跑全绿是 `09ec267` 的树（`spike/NavIconRecolor/gate-95b.log`，套件 8 m 26 s），再往前是 `gate-92.log` 的 1589 条。套件总数只能引管道打印的那一行：`[Theory]` 的 case 也要计进去，拿类别计数做加法一定会算错。**形状别丢**：这之前五次顺序跑各红在一步——页闸红 9 条（`gate-88.log`）/ 套件红 Rating 1 条（`gate-88b.log`）/ 套件红 TeachingTip 1 条（`gate-88c.log`、`gate-94b.log`）/ 页闸红 `materials HighContrast` 2 条（`gate-91.log`）/ 套件红本批自己的 `VisualTreeHelper` 一条（`gate-95.log`），被点名的测点单跑都绿。连着两次全绿**不解释其中任何一条**：`gate-91` 那两条脏底板的复现条件仍未量（形状已收窄到『本页的槽当时还画在宿主上』，见 `## #93 一手`），页闸槽宽仍是整跑一个值、跑与跑之间不同（这跑每腿 788，`pixels-report-2.log` 那跑 39 腿全 1000），#47/#90 原样挂着（日志按批留在 `spike/SystemColorProbe/gate-*.log`、`spike/ForegroundArrival/gate-*.log` 与 `spike/NavIconRecolor/gate-9*.log`、`spike/PagePixelsPlateDiag/gate-93.log`；三档调色板与两份清单每次改动后各自 `-Check` 当场对过） |
...
## #89（目标项 6 的 #88 余账）判掉：把禁用前景格提到 `Style.Triggers` 救不了用户看见的那片墨（2026-09-23）

#88 留下的候选修法是"那一格住在 `ControlTemplate.Triggers` 所以压不过框架的写，提到 `Style.Triggers` 就压得过"。
这条**不做**，因为它的目标面搞错了：提上去只救得到**控件自身的** `Foreground`（7.6b 前两行证得了那里压得过），
而用户看见的禁用文字是**生成的标签**，标签上带的是框架自己的局部值——`AstraForegroundRoutingTests` 三条
`… disabled one comes from our token` 事实钉的就是它：局部值在所有格子之上，我们的格子够不到，只是它现在**取值的名字**
是 `TextDisabled`，被 #12 那行改指到了我们的调色板刷上。两件事合起来的结论是：单独重写
`CheckBoxForegroundUncheckedDisabled` 这一族键，格子住在哪一层都到不了那片像素——**该面没有可用的标记杠杆**，
不是我们漏写了一处。按纪律这记成 Known Gap（`audits/foreground.md` 7.6c-bis），不拿一次"提升格子"的改动冒充结清。

真覆盖那族键今天只有一条路：改 `TextFillColorDisabledBrush` 本身，那会同时改掉所有控件的禁用墨——语义是"全局禁用色"，
不是"这一族键的覆盖点"，所以也不算修法。附带一条风险记录：那次提升会把 hover/pressed 的跨层先后卷进来
（同层"后写的落"量过，跨层没量），在没有可见收益的前提下引入这份不确定不值得。
...
## #92（目标项 6 的 #56 余账第三手）`:210` 两行部件禁用格出事实，`:71` 那行是死格：两个我们的格子相遇时靠后者落格（2026-09-23）

7.6e 剩下能出事实的两行都在 `Styles/TabView.jalxaml:210`（禁用态写 `IconHost` 与 `CloseButton` 两个部件），本批量完它，
并顺手把同一枚 `CloseButton` 上的第二个写者 `:71`（`TabViewCloseButtonStyle` 自己的模板触发器）一起量了。读数见
`audits/foreground.md` 7g，一句结论：**`:71` 一行都到不了像素**——删它 9/9 绿、改指它 9/9 绿，与它同属性的 `:210` 那行
一删就红两条，复合突变（两行同删）的读数和只删 `:210` 一字不差。这条不对称不涉及框架的写、也不涉及局部值，
是"同一属性两格相遇、标记靠后者落格"在我们自己两层之间的一次读数——#89 当初不肯卷进来的跨层先后，这次在零产品改动下量到了。

### 交付形状与撤掉的那条

| 位置 | 干什么 | 为什么 |
|---|---|---|
| `AstraStateCellArrivalTests` 新 2 条 | 禁用 Tab 的 `IconHost` 部件与 `CloseButton` 部件各一条，都先断静态值再断禁用值 | 静态二级/主文字 ≠ 禁用 `#5C000000`，删行就看得见（#91 的判据） |
| 撤掉为 `:71` 写的那条事实 | 它恒真：自己的行删掉、改指都不红任何一条 | 留一条没有属它自己的突变的断言，就是拿 `:210` 的证据冒充 `:71` 的到达性 |
| `mutate.py` 新 7 个突变 | 三格各"删 / 改指成白"，再加 `:210`+`:71` 同删的复合 | `:71` 的 setter 与 `:38` 逐字节相同，needle 必须带整个 trigger 才唯一——仪器当场把这点写进注释 |

### Known Gaps

1. `:38` 那行同键（滚动按钮样式）本批**没量到**：它的载体只在标签条溢出时实现，按上面那条不外推。
2. `:71` 是不是**上游也这样**没量（上游同样有两处写这枚按钮的前景），所以本批只报"这行在我们的运行时到不了像素"，
   不报"这是一处 1:1 失效"。要不要动它，等 #13 的真指针批或单独一次上游对表再判。
3. 这几格是否**真的印出墨**照旧不在断言里（#50）。
...
- **构建**：`dotnet run --project tools/AstraPagePixels` 两条通道都过（`--report` 与判定模式），后者
  ...
  带改动重跑的整条串行闸口也过：`feb45af` 的树上套件 `失败: 0，通过: 1594，总计: 1594`（7 m 48 s）、
  页闸 `PASS … 0 offender(s)`、三档 `checked=True`、两份清单 current，末尾 `gate-exit=0`（`spike/PagePixelsPlateDiag/gate-93.log`）。
