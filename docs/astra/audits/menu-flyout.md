...
14. **菜单图标的换档重绘只有机制，没有读数**（#95，2026-09-23）：三处 `ContentPresenter Name="IconContent"`（`Styles/Menus.jalxaml:68`、`:120`、`:164`）挂上了与应用条同一条 `fluent:IconInk.Source`，但普查（`spike/NavIconRecolor/census-before.log`）在把接线**剪掉**的状态下也没在 `menus` 页量到任何冻结墨——静止时弹层是关着的，图标一个像素都没露过面。所以这三处的依据只有"与应用条同一条码路，而那条码路有事实"，**既没有各自的像素读数，也没有各自的事实**。开着弹层翻档会不会冻，属于 #63 / #13 那套宿主交互未通的部分，本条按"不外推"（`audits/navigation.md` §11）处理。
