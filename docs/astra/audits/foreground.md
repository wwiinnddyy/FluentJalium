...
7g. #92（2026-09-23）把 7.6e 里那两行 `:210` 部件禁用格与 `:71` 的按钮自身样式格量完，形状是 #88 那条不对称的**纯我们内部版**：
   同一支属性上两个我们的格子相遇时，**在标记里靠后的那格落格**，靠前的那格一行都到不了——这一次不涉及框架的写，
   也没有局部值，所以它把 #89 当初"不想卷进来"的跨层先后**在零改动的情况下量出来了**。

   | 那一格 | 位置 | 载体 | 删掉 | 改指成白 | 干净树读数 |
   |---|---|---|---|---|---|
   | `TabViewItemIconForegroundDisabled` | `TabView.jalxaml:210` | 部件 `IconHost` | 只本条红，回落 `#9E000000`（静态那格的二级墨） | 只本条红，读到 `#FFFFFFFF` | `#5C000000` |
   | `TabViewItemHeaderDisabledCloseButtonForeground` | `:210` | 部件 `CloseButton` | **两条**红，回落 `#E4000000`（静态那格的主文字墨） | 两条红，读到 `#FFFFFFFF` | `#5C000000` |
   | `TabViewButtonForegroundDisabled` | `:71`（`TabViewCloseButtonStyle` 自己的模板触发器） | 同一枚 `CloseButton` | **9/9 全绿** | **9/9 全绿** | 与上一行同读数 |
   | 上两行同删（复合突变） | `:210` + `:71` | 同一枚 `CloseButton` | 2 红，回落 `#E4000000`——与只删 `:210` 一字不差 | — | — |

   三条读数要分开看：① 两行部件格都**可判**（静态≠禁用），事实按 #91 的判据出得出来；② `:71` 那行删与不删、
   改指不改指都动不了任何像素，它是**死格**——所以本批**撤掉**了原本为它写的那条事实（它恒真，没有任何属它自己的突变
   能红，留着就是拿相邻证据冒充到达性）；③ `:38` 那行同键、住在 `TabViewButtonStyle`，载体是滚动按钮，只在标签条
   溢出时才实现，本批**没量到**，不许按前两行外推。日志 `spike/ForegroundArrival/mut-tabdis-*.log`。
