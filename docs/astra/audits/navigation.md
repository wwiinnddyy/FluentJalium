...
前后两张抓屏逐像素比"两边都近黑 / 两边都近白"的冻结墨）铺到 Gallery 全部 13 页，读数记在三份文件里：
`spike/NavIconRecolor/census-before.log`（本批接线剪掉后重测）、`census-after.log`（接上"走视觉树"那一版后重测，
那一版没出货）、`census-route-b.log`（接上出货那一版后重测——下表"接上之后"那一列引的就是这一份）：
...
2. **普查的读者是"这一页上有没有不跟档的墨"，不是"这一族有没有接线"**。菜单族三处与 `TabView` 一处接的是同一条
   ...
   写进 Known Gaps。
3. **普查抓的是整屏**，谁盖在窗口上都算墨。三次大数读数（一轮 `settings` 33755、一轮 `surfaces` 28325 + 131、
   出货那一版 `motion` 的 light-in-both 1200）都不重演，而 168 与 519 每次都精确重演；`frozen.ps1` 因此加了
   `-Mask`/`-Crop`，把最脏那个桶的图块留成文件，下次再出现大数就能看而不是猜。`motion` 那一枚一开始连图块都没有：
   裁块只定位 dark-in-both 的桶，而它落在 light-in-both 那一组——补上 light 的桶与裁块后连测两轮 `motion` 各 `0 0`
   （`motion-light.log`），并用 `-Light 0` 把那条新路跑出读数，确认"复测读回 0"不是"新路没跑"。这三次读数不计入
   任何宿主的主张，也不当成本批修好了什么——它们是 #47/#90 那一族"只在某一次跑里现身"的又一个成员。

接线本身还有一处不是像素的账，而且它换过一回。第一版走的是视觉树：附着属性只有一个（`IconInk.Source`），挂在包住
图标的要素上，宿主 `Loaded` 之后广度优先把墨交给身下每一枚 `IconElement`。串行闸口的套件步把它拦下了——
`AstraGateTests` 那条结构闸按 AGENTS.md 的三条禁令把 `VisualTreeHelper` 整个挡在库外（`spike/NavIconRecolor/gate-95.log`，
管道在套件步红并停在那里）。改法不是放宽那条闸，而是换成 `IconInk.Carrier` + `IconInk.Icon` 这一对普通绑定：
不需要"找到元素"，也就没有树可走。换完反而把原来只能写进 Known Gap 的那一条解决了——**子树落过之后才换上去的图标
没人补交墨**现在有人交了，而且钉了一条事实：
`An_icon_swapped_onto_the_host_after_it_was_built_is_handed_the_ink_too`（把回调改成"只认第一个值"即红，
而首帧那条宿主事实照旧绿，`mut-95b.log` 的 `onceonly` 腿）。
