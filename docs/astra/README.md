...
| `adaptation/` | Jalium 宿主能力与偏差 | `00-jalium-theme-capabilities.md`（主题/令牌/VSM/加载路径）、`01-jalium-control-census.md`（163 控件普查）、`02-render-ceiling.md`（IL+DP+像素三重定位真实天花板，并更正 00 的两处过度结论）、`03-material-motion-raw-output.txt`（材质与动效公开面普查）、`04-high-contrast-substitution.md`（高对比改为上游逐键映射，附三处偏差）；各附原始输出 |
