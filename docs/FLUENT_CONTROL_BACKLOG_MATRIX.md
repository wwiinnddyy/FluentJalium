...
| P1 | Material/window QA | DWM/material/window surfaces are a strength and now have Gallery diagnostics for requested profile, actual `Window.SystemBackdrop`, surface role, and material kind. | Validate high contrast fallback, inactive-window material, and unsupported DWM/material hosts in a real Gallery smoke path. | `src/FluentJalium/Controls/Materials/FWMaterials.cs`, `samples/FluentJalium.Gallery/Pages/GalleryWindowBackdropsPage.cs`, `samples/FluentJalium.Gallery/Pages/GalleryMaterialsPage.cs` | Theme resource tests, registry sample assertions, and guarded runtime diagnostics for unsupported DWM/material hosts. |
...
| P1 | Collection navigation depth | `FWListView`, `FWGridView`, state matrix, `FWItemsRepeater`, and `FWAnnotatedScrollBar` exist; current Jalium controls do not expose native `SemanticZoom` or `FlipView` bases for thin FW wrappers. | Evaluate `FWSemanticZoom`, `FWFlipView`, and `FWItemsView` as prototypes or Gallery recipes around existing collection primitives before making public APIs. | `src/FluentJalium/Controls/Collections`, `samples/FluentJalium.Gallery/Pages/GalleryCollectionsPage.cs` | Prototype tests should prove keyboard navigation, selected item semantics, viewport behavior, and virtualization behavior before public API. |
