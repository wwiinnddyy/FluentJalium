...
| P1 | Material/window QA | DWM/material/window surfaces are a strength and have recipes. | Validate real window backdrop application, high contrast fallback, inactive-window material, and derived surfaces in one Gallery diagnostics page. | `src/FluentJalium/Controls/Materials/FWMaterials.cs`, `samples/FluentJalium.Gallery/Pages/GalleryMaterialsPage.cs` | Theme resource tests and guarded runtime diagnostics for unsupported DWM/material hosts. |
| P1 | Advanced interaction completion | `FWRefreshContainer`, `FWScroller`, and `FWAnnotatedScrollBar` now expose diagnostics and Gallery output. | Add richer gesture stories: refresh deferral cancellation, annotation hover/detail presenter, and snap-point visual explanation. | `src/FluentJalium/Controls/Interaction`, `samples/FluentJalium.Gallery/Pages/InteractionControlsPage.cs` | Existing interaction tests plus new event/diagnostic tests for gesture state transitions. |
| P1 | Forms and validation pattern | Inputs, selection, labels, SettingsCard, AutoSuggestBox, and InfoBar exist as building blocks. | Add a Gallery Forms pattern page for label alignment, validation messages, required fields, async submit, and density. | `samples/FluentJalium.Gallery/Pages`, `GalleryCatalog.cs`, `GalleryLocalizationService.cs` | Catalog factory test and sample-code registry assertions; no new public control unless a reusable `FWForm` abstraction proves necessary. |
| P1 | Collection navigation depth | `FWListView`, `FWGridView`, state matrix, `FWItemsRepeater`, and `FWAnnotatedScrollBar` exist. | Evaluate `FWSemanticZoom`, `FWFlipView`, and `FWItemsView` as separate controls versus Gallery recipes around existing collection primitives. | `src/FluentJalium/Controls/Collections`, `samples/FluentJalium.Gallery/Pages/GalleryCollectionsPage.cs` | Prototype tests should prove keyboard navigation, selected item semantics, and virtualization behavior before public API. |
...
| --- | --- | --- | --- |
| P0 | Keep All/New/Updated/Preview/Diagnostic filters complete | WinUI Gallery-style discovery is now part of the product surface. | Every `GalleryCatalog` entry has factory, related controls, docs links, source path, and sample key when applicable. |
...
| P1 | Code/sample split | FluentAvalonia and WinUI Gallery both make samples inspectable; FluentJalium should keep registry code accurate. | `GallerySampleCodeRegistry` has assertions for new scenario keys and does not drift from page behavior. |
| P1 | Visual QA checklist per control family | Many remaining gaps are visual/state confidence, not missing types. | Each family has normal, hover, pressed, selected, disabled, focus, light/dark/high contrast, and density coverage where meaningful. |
...
2. Forms pattern Gallery page: no new public API required, high user value, ties together labels, text input, selection, InfoBar, SettingsCard, and validation states.
3. Navigation app-shell walkthrough: demonstrates WPF UI/UI.WPF.Modern-inspired shell ergonomics with FluentJalium-native route diagnostics.
4. Collection navigation evaluation: decide whether `FWSemanticZoom`, `FWFlipView`, and `FWItemsView` should become public controls or remain recipes.
