...
| --- | --- | --- | --- | --- | --- |
| P0 | TaskDialog root-window smoke | `FWTaskDialog`, host, automation, keyboard diagnostics, Gallery samples, and a real-window QA snapshot now cover modal layering, focus, keyboard, automation, light dismiss, and command readiness. | Validate the same matrix in a full root-window smoke path with richer close/restore timing and visual clipping coverage. | `src/FluentJalium/Controls/Disclosure/FWDisclosureControls.cs`, `samples/FluentJalium.Gallery/Pages/GalleryDisclosurePage.cs`, `Themes/Controls/DisclosureControls.jalxaml` | Existing headless template tests plus a future root-window smoke path that exercises host show/close/focus diagnostics. |
...
1. TaskDialog root-window smoke: build on the new real-window QA snapshot and validate close/restore timing in a full Gallery host.
